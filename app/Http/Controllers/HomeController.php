...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        //$summer_banners = json_decode(get_setting('summer_banners'), true);
        if (app()->getLocale() == 'sa') {
            $summer_banners = ["assets/front_img/sa-summer-product.jpeg", "assets/front_img/sa-summer-product1.jpg", "assets/front_img/sa-summer-product2.jpeg"];
        } elseif (app()->getLocale() == 'en') {
            $summer_banners = ["assets/front_img/en-summer-product.png", "assets/front_img/en-summer-product1.png", "assets/front_img/en-summer-product2.jpg"];
        } else {
            $summer_banners = ["assets/front_img/cn-summer-product.png", "assets/front_img/cn-summer-product1.png", "assets/front_img/cn-summer-product2.jpg"];
        }

        $best_selling_products = Product::where('published', 1)
        ...
        $deal_of_the_week = Product::where('deal_of_the_day', 1)
            ->latest()
            ->take(10)
            ->get();

        $grill_banner = get_slider_images(json_decode(get_setting('home_banner3_images', null, $lang), true));
        if($grill_banner)
    ...
    {
        $company = Company::updateOrCreate(['user_id'=>Auth::user()->id,],[
            'company_name' => $request->company_name,
            'tax_number' => $request->tax_number,
            'commercial_register' => $request->commercial_register,
            'created_at' => now(),
        ]);
        flash(translate('Company Updated!'))->success();
        return redirect()->back();
    }
    public function service_request()
    {
        $products = Product::where('category_id', 76)->get()->take(6);
        return view('frontend.classic.service_request', compact('products'));
    }
    ...
    public function service_request_store(Request $request)
    {
        ...
        $request->validate([
            'file'      => 'required|file|mimes:jpeg,png,jpg,pdf|max:20480',
        ...
        try {
            if ($request->hasFile('file')) {
                ...
                $destinationPath = public_path('service_request_upload');
                ...
                $path = 'service_request_upload/' . $filename;
            } else {
                $path = null;
            }
            ...
            return redirect()->back()->with('success', translate("success_message"));
        } catch (\Exception $e) {
            \Log::error('Service request failed: ' . $e->getMessage(), ['exception' => $e]);
            return redirect()->back()->withInput()->with('error', translate('Something went wrong. Please try again.'));
        }
    }

    public function quotation_request_store(Request $request)
    {
        // return view('frontend.classic.thankyou');
        $user = auth()->user();
        ...
        if ($user->is_company != 1) {
            if (app()->getLocale() == "sa")
                flash("عذرًا، أنت غير مسجل كشركة")->error();
            elseif (app()->getLocale() == "cn")
                flash("抱歉，您尚未注册为公司用户")->error();
            else
                flash("Sorry,You are not Registered as a Company")->error();
        ...
        if ($company->company_name == null || $company->tax_number == null || $company->commercial_register == null) {

            if (app()->getLocale() == "sa")
                flash("يرجى أولاً تعبئة جميع بيانات الشركة")->error();
            elseif (app()->getLocale() == "cn")
                flash("请先填写完整的公司信息")->error();
            else
                flash("Please fill in all your company data first")->error();
        ...
        foreach ($carts as $cart) {
            ...
            $items[] = [
                ...
                'price' => cart_product_price($cart, $product, false, false),
        ...
        $pdf = PDF::loadView('frontend.classic.request_quote_pdf', [
            'items' => $items,
        ...
        Mail::to($user->email)->send(new RequestQuoteMail($pdf->output()));

        $quote = new Quotation();
        $quote->user_id = $user->id;
        $quote->pdf = json_encode($pdf->output());
        $quote->created_at = now();
        $quote->save();

        if (app()->getLocale() == "sa")
            flash('تم إرسال طلب عرض السعر إلى بريدك الإلكتروني بنجاح')->success();
        elseif (app()->getLocale() == "cn")
            flash('报价请求已成功发送至您的电子邮箱')->success();
        else
            flash('The quotation request has been successfully sent to your email')->success();
    ...
    public function send_quote(Request $request)
    {
        ...
        if ($request->hasFile('invoice_file')) {
            $filePath = $request->file('invoice_file')->store('uploads/invoice_files', 'public');
        }
        ...
        return back()->with('success', translate('Your maintenance request has been submitted successfully.'));
    }
    ...
    //         'file' => 'nullable|file|max:10240' // max 10MB مثلا
    //     ]);

    //     // handle file upload
    //     $filePath = null;
    //     if ($request->hasFile('file')) {
    //         $filePath = $request->file('file')->store('uploads/quote_files', 'public');
    //     }
    ...
    //     return back()->with('success', translate('Your quote request has been submitted successfully.'));
    // }
    ...
    public function faq()
    {
        ...
        $faqs = [
                    [
                        'question' => translate('Lorem Lorem Lorem'),
                        'answer' => translate('lorem ipsum'),
                    ],
                    [
                        'question' => translate('Lorem Lorem Lorem'),
                        'answer' => translate('lorem ipsum'),
        ...
        return view('frontend.classic.faq',compact('faqs'));
    ...
    public function all_our_partners(){
        $all_partners = Partner::all();
        return view('frontend.partners' , compact('all_partners'));
    }
}
