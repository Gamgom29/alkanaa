...
class CheckoutController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if (auth()->check()) {
            ...
            $temp_user_id = $this->getTempUserId($request);
        ...
        if ($carts && count($carts) > 0) {
            ...
            return view('frontend.checkout', compact('carts', 'service_request', 'address_id', 'total', 'carrier_list', 'shipping_info'));
        }
        flash(translate('Your cart is empty'))->warning();
        return redirect()->route('home');   // لو مفيش route('cart')
    ...
    public function checkout(Request $request)
    {
        ...
        $combinedId = $request->session()->get('combined_order_id');
        if (!$combinedId) {
        ...
        if ($request->service_request == "1") {
            $productIds = array_unique(array_map(fn($item) => $item['product_id'], $carts->toArray()));
        ...
        $onlineMethods = ['moyasar', 'stripe', 'paypal', 'paytabs']; // عدّل حسب نظامك



        if ($paymentMethod === 'cash_on_delivery') {
            // سيب الأوردر unpaid أو pending حسب نظامك
            // وامسح الكارت لأنه مش Online
        ...
        // لو Moyasar -> ودّيه لصفحة الفورم
        if ($paymentMethod === 'moyasar') {
            return redirect()->route('moyasar.form', ['combined_order_id' => $combinedId]);
        }

        // if (!in_array($paymentMethod, $onlineMethods)) {
        //     $carts->toQuery()->delete();
        // }
    ...
    public function createUser($guest_shipping_info)
    {
        ...
        $tempId = request()->session()->get('temp_user_id') ?? request()->cookie('temp_user_id');
    ...
    public function get_shipping_info(Request $request)
    {
        ...
        flash(translate('Your cart is empty'))->warning();
        return redirect()->route('home');   // أو route('cart')
    ...
    {
        $temp = $request->session()->get('temp_user_id');

        if (!$temp) {
            $temp = $request->cookie('temp_user_id');
            if ($temp) {
                $request->session()->put('temp_user_id', $temp); // ثبّت الكوكي في السيشن
            }
        }
    ...
    public function updateDeliveryAddress(Request $request)
    {
        ...
        $carts = $user != null ?
            ...
            Cart::where('temp_user_id', $this->getTempUserId($request))
            ->active()->get();
